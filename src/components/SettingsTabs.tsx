    ...
    <div>
      <div className="flex items-center gap-1 border border-gray-200 rounded-lg p-1 mb-6 w-fit">
        <button
          ...
          className={`text-sm px-3 py-1.5 rounded-md ${tab === 'profile' ? 'bg-[#378ADD] text-white' : 'text-gray-600 hover:text-gray-900'}`}
        >
          Profile
        </button>
        <button
          onClick={() => setTab('application')}
          className={`text-sm px-3 py-1.5 rounded-md ${tab === 'application' ? 'bg-[#378ADD] text-white' : 'text-gray-600 hover:text-gray-900'}`}
        >
          Application
        </button>
      </div>
