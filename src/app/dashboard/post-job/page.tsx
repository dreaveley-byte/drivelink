...
export default function PostJobPage() {
  ...
  const runCalculation = useCallback(async (scheduledForOverride?: string, forceFlying?: boolean, precomputedFlyCharges?: AdditionalCharge[] | null) => {
    ...
    try {
      ...
      // separate manual entries apart. Keeps the LAST occurrence of each
      // kind, since fresh auto-generated charges are always appended after
      // manualCharges in the array order.
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data: newJob, error: jobError } = await supabase.from('jobs').insert({
      ...
      driver_paid_hours: primaryDriverPricing?.driverPaidHours ?? null,
    ...
    if (secondDriver && !isDealerToDealerMultiVehicle && pricingSettings && distanceKm != null && durationMinutes != null) {
      ...
      insurance_visit: chaseVehicle ? false : insuranceVisit,
        ...
        estimated_duration_minutes: durationMinutes,
        driver_paid_hours: soloPricing.driverPaidHours,
