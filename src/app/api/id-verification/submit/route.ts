...
async function validatePhoto(base64DataUrl: string, kind: 'face' | 'license'): Promise<{ ok: boolean; reason?: string }> {
  ...
  const prompt =
    kind === 'face'
      ...
      : 'This should be a clear, sharply in-focus photo of a government-issued photo ID or driver\'s license. Be strict: every line of printed text (name, address, date of birth, ID number, expiry date) must be sharp enough to read with confidence, not just roughly make out — any noticeable blur, motion blur, low resolution, glare, shadow, or a steep/angled shot that distorts the text counts as a failure, even if the general shape and photo of the ID are visible. When genuinely uncertain whether text is crisp enough, reply with the failure reason rather than "OK" — a retake is cheap, but accepting an unreadable ID is not. Reply with ONLY "OK" if it clearly meets that strict standard. Otherwise reply with ONLY a short reason (under 12 words) why it does not (e.g. "too blurry to read", "text not sharp enough", "no ID visible", "glare obscuring text", "shot at an angle", "not a government ID").'
