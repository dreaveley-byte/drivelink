...
export default function DriverJobActions({
  ...
  // deliveryAckAccepted only ever lived in memory - lost on every page
  // reload, with nothing checking whether the acknowledgement had already
  // been agreed to. That's the actual bug behind "it worked, but now I
  // can't get back in for the signature": after the agree step succeeded,
  // any reload (or the app's own periodic refresh) wiped the in-memory
  // flag, so the screen fell back to showing the original "Review & Sign"
  // button as if nothing had happened - with no way to reach the
  // signature pad specifically, since that pad's visibility depended on
  // this same lost flag. Checking the database directly on mount fixes
  // that: the acknowledgement's agree-step state now survives reloads.
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setDeliveryAckMediaConsent(data.media_consent)
        }
      })
    // Only ever needs to run once per job, on mount - not on every
    // job-data refresh elsewhere in this component.
                      ...
                      <div className="mt-2">
                        ...
                        {item.item_type === 'signature' && (
                          <div className="space-y-2">
                                  ...
                                  <div className="space-y-2">
                                    {item.completed_at ? (
                                      <p className="text-xs text-green-600 bg-green-50 border border-green-200 rounded-lg p-3">
                                        ✓ Vehicle Delivery Acknowledgement reviewed, agreed to, and signed by the customer
                                      ...
                                      <p className="text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-300 rounded-lg px-3 py-2">
                                        ✓ Acknowledgement agreed to — one last step: have the customer sign below to finish.
                                      </p>
                                    ) : (() => {
                                      const idVerificationReady =
                                        job.id_verification_manual_override ||
                            ...
                            {(item.label !== 'Delivery: Customer signs delivery disclosure' || (deliveryAckAccepted && !item.completed_at)) && (
