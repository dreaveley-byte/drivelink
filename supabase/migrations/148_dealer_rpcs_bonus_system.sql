-- Updates both dealer RPCs for the new performance bonus system:
-- 1. Food is included in approved expenses again (no longer excluded) -
--    driver pay no longer bakes in a food component, so there's no more
--    double-counting risk.
-- 2. Performance bonus (the driver's share of unused meal budget, now
--    conditional and shown separately) is included as a real cost too,
--    since it's money actually paid out.
  ...
  job_financials as (
    select
      ...
      coalesce(j.admin_pay_override_cents, j.final_driver_pay_cents, j.estimated_driver_pay_cents, 0)
        ...
        + coalesce(j.performance_bonus_override_cents, j.performance_bonus_cents, 0) as actual_cost_cents
    from jobs j
...
as $$
  ...
  select
    ...
    coalesce(j.performance_bonus_override_cents, j.performance_bonus_cents, 0) as performance_bonus_cents,
    ...
    (coalesce(j.admin_pay_override_cents, j.final_driver_pay_cents, j.estimated_driver_pay_cents, 0)
      + coalesce(jet.approved_full_amount_cents, 0)
      + coalesce(j.performance_bonus_override_cents, j.performance_bonus_cents, 0)) as total_cost_cents,
    ...
    (coalesce(j.estimated_dealer_cost_cents, 0) + coalesce(jet.approved_additions_cents, 0))
      - (coalesce(j.admin_pay_override_cents, j.final_driver_pay_cents, j.estimated_driver_pay_cents, 0)
        + coalesce(jet.approved_full_amount_cents, 0)
        + coalesce(j.performance_bonus_override_cents, j.performance_bonus_cents, 0)) as profit_cents
  from jobs j
