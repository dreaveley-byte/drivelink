alter table job_expenses add column if not exists receipt_date date;
alter table job_expenses add column if not exists looks_legitimate boolean not null default true;
alter table job_expenses add column if not exists auto_approved boolean not null default false;
...
begin
  ...
  if NEW.category = 'food' then
    v_add_amount := 0;
  elsif NEW.category in ('fuel', 'inspection', 'hotel') then
