-- Archiving a job has only ever removed it from the active job list - it
-- was never excluded from payroll or earnings math anywhere. That's the
-- real bug behind the inflated payroll totals: every one of the test/seed
-- jobs was already correctly archived, but archived_at was never checked
-- by any of the financial queries, so they kept counting toward driver
-- pay, dealer spend, and company profit regardless.
