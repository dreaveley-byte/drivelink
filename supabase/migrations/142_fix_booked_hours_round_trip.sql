-- Fixes the same bug just found on the individual job receipt page:
-- booked_minutes was estimated_duration_minutes, which is only ever the
-- raw one-way Google Maps drive time - not the actual round-trip-aware
-- hours a vehicle delivery driver is paid for. Replaced with
-- driver_paid_hours directly (the real figure computed by the pricing
-- engine and stored at posting/edit time), falling back to the old
-- one-way-only estimate for older jobs that predate that column.
